...
import {
  ...
  findEveryDaySongs,
  findTopGenres,
  calculateListeningTimeBySong,
  calculateListeningTimeByArtist,
  calculateFridayNightListeningTime,
...
function onUserSelected(userId) {
  ...
  if (userData.length === 0) {
    ...
    updateResultsTable(userId, {}); // Clear UI if no data
    return;
  }
  ...
  console.log(
    mostPlayedSong
      ? `Most listened song (count): ${
          getSong(mostPlayedSong.song_id).artist
        } - ${getSong(mostPlayedSong.song_id).title}`
  ...
  const mostPlayedSongTime = Object.entries(listeningTimeData).reduce(
  ...
  console.log(
    fridayNightSong
      ? `Friday night song (count): ${
          getSong(fridayNightSong.song_id).artist
        } - ${getSong(fridayNightSong.song_id).title}`
  ...
  console.log(
    longestStreak
      ? `Longest streak song: ${getSong(longestStreak.song_id).artist} - ${
          getSong(longestStreak.song_id).title
        } (length:${longestStreak.streak})`
